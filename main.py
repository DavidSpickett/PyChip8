...
class ChipEightSystem(object):
    ...
    def do_next_opcode(self):
        ...
        '''
        next_code = OpCode(self.memory[self.pc_reg] + self.memory[self.pc_reg+1])
        ...
        #Set Vx = Vx - Vy, set VF = NOT borrow.
            ...
            self.v_regs[15] = 0 #Set VF to 0, we'll check if it needs to be 1 next
        ...
        #Set Vx = Vx SHL 1.
            ...
            self.v_regs[to_int(next_code[1])] = (self.v_regs[to_int(next_code[1])] << 1) & 255
        ...
        #Set Vx = random byte AND kk.
            ...
            self.v_regs[to_int(next_code[1])] = random.randint(0, 255) & to_int(next_code[2:4])
            self.pc_reg += 2
        ...
        #This is done using an XOR function, so writing to an already set pixel will erase it.
            ...
            #Set to indicate no overwrites, we will set it if and when one occurs
            self.v_regs[15] = 0
                ...
                for j, pixel in enumerate(sprite_row): #0s and 1s in each row
                    ...
                    if pixel == "1":
                        if self.display[new_x][new_y]: #XOR means we overwrite a pixel
                            ...
                            self.v_regs[15] = 1 #Indiciate overwriting occured
                        else:
        ...
        #Set I = location of sprite for digit Vx.
            ...
            self.i_reg = self.v_regs[to_int(next_code[1])]*5 #Each char bieng 5 bytes long
            self.pc_reg += 2

        #Fx33 - LD B, Vx
        #Store BCD representation of Vx in memory locations I, I+1, and I+2.
        elif next_code == "f?33":
            number = self.v_regs[to_int(next_code[1])] #The number we need to process
...
if __name__ == "__main__":
    ...
    pygame.time.set_timer(pygame.USEREVENT+1, 17) #60Hz timer for decrementing delay registers

    try:
        while True:
            cpu.do_next_opcode() #Fetch next opcode
            ...
            if event.type == pygame.USEREVENT+1:
                #Timer register
                if cpu.delay_timer > 0: #Decrement until 0
                    cpu.delay_timer -= 1

                #Sound register
                if cpu.sound_timer > 0: #Decrement until 0
                    cpu.sound_timer -= 1
